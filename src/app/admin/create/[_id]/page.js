...
function Create(props) {
  ...
  const handleChangeFile = async (event) => {
    try {
        ...
        setImgUrl(`https://usedtech-fb06c88c0961.herokuapp.com${data.url}`);
  ...
  const handleChangeFiles = async (event) => {
    try {
        ...
        const modifiedUrls = data.urls.map(url => 'https://usedtech-fb06c88c0961.herokuapp.com' + url);
  ...
  const deleteImage = async () => {
    const urlToDelete = imgUrl.replace('https://usedtech-fb06c88c0961.herokuapp.com/uploads', '');
    try {
  ...
  const handleDeleteImage = async (event, index) => {
    ...
    const urlToDelete = imgUrls[index].replace('https://usedtech-fb06c88c0961.herokuapp.com/uploads', '');
